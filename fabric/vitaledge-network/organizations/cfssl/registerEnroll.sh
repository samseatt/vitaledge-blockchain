...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.drx.network/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.drx.network/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.drx.network/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.drx.network/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.drx.network/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.drx.network

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.drx.network-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.drx.network-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.drx.network

    mkdir -p organizations/ordererOrganizations/drx.network/ca
    mkdir -p organizations/ordererOrganizations/drx.network/msp/cacerts
    mkdir -p organizations/ordererOrganizations/drx.network/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/drx.network/orderers
    mkdir -p organizations/ordererOrganizations/drx.network/tlsca

    CERT_DIR=organizations/ordererOrganizations/drx.network

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.drx.network-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.drx.network-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/drx.network/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/drx.network/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.drx.network-cert.pem"
